...
class InvoicePdf < Prawn::Document
  ...
  def company_address
    grid([1, 3],[2,4]).bounding_box do
      move_down 10
      text "#{@invoice.line_1}", size: 13, style: :bold, align: :left
      move_down 10
      text "#{@invoice.line_2}", size: 12, align: :left
      text "#{@invoice.line_3}", size: 12, align: :left
      text "#{@invoice.line_4}", size: 12, align: :left
    end
  end
