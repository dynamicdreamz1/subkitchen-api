describe Sessions::Api, type: :request do
  describe '/api/v1/sessions' do
    it 'should be able to authenticate' do
      ...
      post '/api/v1/sessions', token: user.password_reminder_token
    ...
    it 'should be able to sign in' do
      expect do
        post '/api/v1/sessions/sign_in', email: 'test@test.pl'
      ...
      describe 'facebook' do
        it 'should not be able to connect with invalid token' do
          VCR.use_cassette('oauth/facebook/invalid') do
            expect do
              post '/api/v1/sessions/oauth/facebook', access_token: 'asd'
        ...
        it 'should be able to connect' do
          VCR.use_cassette('oauth/facebook/valid') do
            expect do
              post '/api/v1/sessions/oauth/facebook', access_token: 'test'
            end.to change(User, :count).by(1)
            expect(response).to have_http_status(:success)
            user = User.last
            expect(response.body).to eq(user.to_json)
